...
with open("/tmp/cgu_opt_levels.txt", "w") as f:
    ...
    for crate in known_crates:
        ...
        if n == 0: continue
        avg = total / n
        if avg > hot_thr: level = "O3"
        elif avg > warm_thr: level = "O2"
        elif avg > tepid_thr: level = "Os"
        else: level = "Oz"
        f.write(f"{crate} {level}\n")

print(f"Wrote /tmp/fn_opt_levels.txt ({sum(1 for c in counts.values() if c > tepid_thr)} classified)")
print(f"Wrote /tmp/cgu_opt_levels.txt ({sum(1 for _, n in crate_counts.values() if n > 0)} crates)")
