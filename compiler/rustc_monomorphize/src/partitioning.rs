        ...
        if let Some(ref hot_func_path) = tcx.sess.opts.unstable_opts.hot_function_list {
            ...
            for cgu in codegen_units.iter_mut() {
                ...
                if cold_items.is_empty() {
                    ...
                    // Mixed CGU: split into .o3 (hot) and .oz (cold) parts
                    ...
                    for (item, data) in &hot_items { hot_cgu.items_mut().insert(*item, *data); }
                    ...
                    for (item, data) in &cold_items { cold_cgu.items_mut().insert(*item, *data); }
